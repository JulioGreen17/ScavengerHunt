...
class TaskDetailViewController: UIViewController {
    ...
    @IBAction func didTapAttachPhotoButton(_ sender: Any) {
        let alert = UIAlertController(title: "Add a Photo", message: "Choose a source", preferredStyle: .actionSheet)
...
extension TaskDetailViewController: PHPickerViewControllerDelegate {
    ...
    private func presentCamera() {
        guard UIImagePickerController.isSourceTypeAvailable(.camera) else {
            showAlert(for: NSError(domain: "Camera not available", code: 1))
            return
        }
...
extension TaskDetailViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        // Set the picked image on the task without a location (since camera images may not have it)
        task.set(image, with: nil)
